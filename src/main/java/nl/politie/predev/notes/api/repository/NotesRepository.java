...
public interface NotesRepository extends RefreshableRepository<Note, Long> {
	...
	@Query("SELECT n FROM Note n LEFT JOIN SharedNote s on n.noteID=s.noteID WHERE (n.owner =?1 OR (s.sharedWithUsername= ?1 OR s.sharedWithGroupname IN (?2) AND s.isDeleted = false)) AND n.is_deleted = false ORDER BY n.generated_at DESC")
